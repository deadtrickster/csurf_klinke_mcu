...
void (*Undo_OnStateChangeEx)(const char *descchange, int whichStates, int trackparm); // trackparm=-1 by default, or if updating one fx chain, you can specify track index
...
#define IMPVARP(x,nm,m_type) if (!((x) = projectconfig_var_getoffs(nm,&sztmp)) || sztmp != sizeof(m_type)) errcnt++;
